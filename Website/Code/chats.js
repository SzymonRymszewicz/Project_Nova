...
const CHAT_GROUP_FALLBACK_STYLE = 'background:linear-gradient(135deg,#19f2ff 0%,#ff2b9a 100%);';

function postChatsJson(url, payload) {
	return fetch(url, {
		method: 'POST',
...
function getChatGroupVisual(botName, bot) {
	...
	const coverStyle = iconArt
		...
		: CHAT_GROUP_FALLBACK_STYLE;
	const coverContent = iconArt ? '' : `<div>${initial}</div>`;
	return { coverStyle, coverContent };
}

function getChatMetaText(chat) {
	const totalMessages = Number.isInteger(chat.total_message_count)
		? chat.total_message_count
		: (Number.isInteger(chat.message_count) ? chat.message_count : 0);
	const userMessages = Number.isInteger(chat.user_message_count) ? chat.user_message_count : null;
	return userMessages === null
		? `Messages: ${totalMessages}`
		: `Messages: ${totalMessages} (You: ${userMessages})`;
}
...
function showChats() {
	...
	Promise.all([
		...
		orderedBots.forEach(botName => {
			...
			const { coverStyle, coverContent } = getChatGroupVisual(botName, bot);
			...
			const metaText = getChatMetaText(chat);
			item.innerHTML = `<div class="chat-item-icon chat-item-icon-small" style="${coverStyle}">${coverContent}</div><div class="chat-item-content"><div class="chat-item-title-row"><div class="chat-item-title">${chat.title}</div>${isLastOpened ? '<span class="chat-last-opened-badge">Last opened</span>' : ''}</div><div class="chat-item-meta">${metaText}</div></div><button class="chat-delete-btn" type="button" title="Delete chat" aria-label="Delete chat"><svg viewBox="0 0 20 20" fill="currentColor"><path fill-rule="evenodd" d="M8 2a1 1 0 00-1 1v1H5a1 1 0 100 2h.293l.853 10.24A2 2 0 008.14 18h3.72a2 2 0 001.994-1.76L14.707 6H15a1 1 0 100-2h-2V3a1 1 0 00-1-1H8zm3 2V4H9V4h2zm-2 4a1 1 0 10-2 0v6a1 1 0 102 0V8zm4-1a1 1 0 00-1 1v6a1 1 0 102 0V8a1 1 0 00-1-1z" clip-rule="evenodd"/></svg></button>`;
...
function deleteChat(chatId, botName, chatTitle) {
	...
	postChatsJson('/api/chats', { action: 'delete', chat_id: chatId, bot_name: botName })
		.then(r => r.json())
		.then(result => {
...
function openChat(chatId, botName) {
	...
	postChatsJson('/api/bot/select', { bot_name: botName })
		.then(r => r.json())
		.then(botData => {
			if (botData.success) {
			...
			// Then load the chat
			return postChatsJson('/api/load-chat', { chat_id: chatId, bot_name: botName });
		})
		.then(r => r.json())
		.then(data => {
